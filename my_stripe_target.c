...
static int my_stripe_ctr(struct dm_target *ti, unsigned int argc, char **argv) {
    ...
    ti->private = ctx;
    printk(KERN_INFO "my_stripe: constructor complete.\n");
    return 0;
}
...
static void my_stripe_dtr(struct dm_target *ti) {
    ...
    kfree(ctx);
    printk(KERN_INFO "my_stripe: destructor complete.\n");
}
...
static int __init my_stripe_init(void) {
    ...
    if (result < 0) {
        ...
        printk(KERN_INFO "my_stripe target registered.\n");
    }
    return result;
}
...
static void __exit my_stripe_exit(void) {
    ...
    printk(KERN_INFO "my_stripe target unregistered.\n");
}
